...
def main(params):
    ...
    print("FINAL seed:", params["seed"])
    set_seed(params["seed"])

    if params['use_wandb']==1:
        import wandb
        wandb.init(config=params)
        wandb.config.update({"seed": params["seed"]}, allow_val_change=True)
    ...
    with open("../configs/kt_config.json") as f:
        ...
        if model_name in ["dkvmn","deep_irt", "sakt", "saint","saint++" , "robustkt", "folibikt", "atkt", "lpkt", "skvmn", "dimkt"]:
    ...
    print("FINAL train_config seed:", train_config["seed"])
    print("FINAL train_config batch_size:", train_config["batch_size"])
    ...
    if save_model:
        ...
        test_data_config = copy.deepcopy(data_config[dataset_name])
        test_data_config["dataset_name"] = dataset_name
        test_loaders = init_test_datasets(
            test_data_config,
            model_name,
            batch_size
        )
        ...
        test_window_loader = test_loaders[1]

        testauc, testacc = evaluate(best_model, test_loader, model_name)
        window_testauc, window_testacc = evaluate(best_model, test_window_loader, model_name)

        print("TEST RESULT:")
        print("testauc:", testauc)
        print("testacc:", testacc)
        print("window_testauc:", window_testauc)
        print("window_testacc:", window_testacc)
