...
class AKTOurs(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks, dropout, d_ff=256, 
            kq_same=1, final_fc_dim=512, num_attn_heads=8, separate_qa=False, l2=1e-5, emb_type="qid", emb_path="", pretrain_dim=768, num_groups=9):
        super().__init__()
        """
        Input:
            ...
            num_grouups: number of student groups, e.g. 8
        """
        self.model_name = "akt_ours"
        ...
        # =========================================================
        # Ours: group embedding + alphq gate
        # =========================================================

        self.num_groups = num_groups
        # group_id: [B] 或 [B, L]
        # group_embed_data: [B, L, d_model]
        self.group_emb = nn.Embedding(num_groups, d_model)
        # alpha_feat from dataloader: [B, L, 3]
        # distance from model: [B, L, 1]
        # u = concat(distance, alpha_feat): [B, L, 4]
        ...
        # make alpha initially close to 1, so the model starts near original AKT
        nn.init.constant_(self.alpha_mlp[-2].bias, 3.0)
    ...
    def forward(self, q_data, target, pid_data=None, group_id=None, alpha_feat=None,qtest=False):
        ...
        q_data:     [B, L]
        target:     [B, L]
        pid_data:   [B, L] or None

        group_id:
            [B]    表示每个学生一个 group
            [B, L] 表示每个 timestep 一个 group
            None   默认全部设为 group 0，只是为了兼容旧训练流程

        alpha_feat:
            [B, L, 3] 推荐格式，包含 log_hist_len、recent_volatility、behavior_risk
            模型内部会计算 distance，并拼接成 [B, L, 4]
            None      默认使用全 0 特征，让 alpha_mlp 学一个全局 gate
        """
        ...
        # =========================================================
        # Ours: causal group-aware residual shrinkage after AKT encoder
        # =========================================================
        B, L, D = d_output.size()

        # group_id: None -> unknown group
        if group_id is None:
            group_id = torch.full(
                (B, L),
                self.num_groups - 1,
            ...
            group_id = group_id.to(d_output.device).long()

            if group_id.dim() == 1:
                group_id = group_id.unsqueeze(1).expand(-1, L)
            elif group_id.dim() == 2 and group_id.size(1) == 1:
                group_id = group_id.expand(-1, L)
            elif group_id.dim() != 2:
                raise ValueError(
                    f"group_id should have shape [B] or [B, L], but got {group_id.shape}"
                )

            if group_id.size(0) != B or group_id.size(1) != L:
                raise ValueError(
                    f"group_id shape should match hidden shape [B, L]. "
                    f"Got group_id={group_id.shape}, hidden={d_output.shape}"
                )

        group_id = torch.clamp(group_id, min=0, max=self.num_groups - 1)

        g = self.group_emb(group_id)  # [B, L, D]

        delta = d_output - g
        distance = torch.norm(delta, p=2, dim=-1, keepdim=True) / (D ** 0.5)   # [B, L, 1]

        # alpha_feat from dataloader should be [B, L, 3]
        if alpha_feat is None:
            alpha_feat = torch.zeros(
                B,
                L,
                3,
            ...
            alpha_feat = alpha_feat.to(d_output.device).float()

            if alpha_feat.dim() == 2:
                alpha_feat = alpha_feat.unsqueeze(1).expand(-1, L, -1)

            if alpha_feat.dim() != 3:
                raise ValueError(
                    f"alpha_feat should have shape [B, L, 3] or [B, 3], "
                    f"but got {alpha_feat.shape}"
                )

            if alpha_feat.size(0) != B or alpha_feat.size(1) != L:
                raise ValueError(
                    f"alpha_feat shape should match hidden shape [B, L]. "
                    f"Got alpha_feat={alpha_feat.shape}, hidden={d_output.shape}"
                )

            if alpha_feat.size(-1) != 3:
                raise ValueError(
                    f"alpha_feat last dim should be 3, but got {alpha_feat.size(-1)}"
                )

        u = torch.cat([distance, alpha_feat], dim=-1)  # [B, L, 4]
        alpha = self.alpha_mlp(u)                    # [B, L, 1]
        #alpha = torch.full_like(distance, 0.90)
        ...
        # residual shrinkage:
        # alpha = 1 -> keep individual AKT state
        # alpha = 0 -> shrink to group baseline
        d_output = g + alpha * delta
