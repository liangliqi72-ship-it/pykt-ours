...
def evaluate(model, test_loader, model_name, rel=None, save_path=""):
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            # ours: causal group information
            group_id = dcur.get("group_id", None)
            shft_group_id = dcur.get("shft_group_id", None)

            alpha_feat = dcur.get("alpha_feat", None)
            shft_alpha_feat = dcur.get("shft_alpha_feat", None)

            if group_id is not None:
                group_id = group_id.long().to(device)

            if shft_group_id is not None:
                shft_group_id = shft_group_id.long().to(device)

            if alpha_feat is not None:
                alpha_feat = alpha_feat.float().to(device)

            if shft_alpha_feat is not None:
                shft_alpha_feat = shft_alpha_feat.float().to(device)
            #===============================================
            ...
            # ours: recover full-length causal group sequence for AKTOurs
            cgroup_id = None
            calpha_feat = None
            ...
            if model_name in ["akt_ours", "akt_dugp"]:
                if group_id is not None:
                    if group_id.dim() == 1:
                        # old scalar fallback, normally not used now
                        cgroup_id = group_id.unsqueeze(1).expand(-1, cc.size(1))
                    elif group_id.size(1) == cc.size(1):
                        cgroup_id = group_id
                    elif shft_group_id is not None and group_id.size(1) == cc.size(1) - 1:
                        cgroup_id = torch.cat((group_id[:, 0:1], shft_group_id), dim=1)
                    else:
                        raise ValueError(
                            f"group_id length mismatch: group_id={group_id.shape}, "
                            f"shft_group_id={None if shft_group_id is None else shft_group_id.shape}, "
                            f"cc={cc.shape}"
                        )

                if alpha_feat is not None:
                    if alpha_feat.size(1) == cc.size(1):
                        calpha_feat = alpha_feat
                    elif shft_alpha_feat is not None and alpha_feat.size(1) == cc.size(1) - 1:
                        calpha_feat = torch.cat((alpha_feat[:, 0:1, :], shft_alpha_feat), dim=1)
                    else:
                        raise ValueError(
                            f"alpha_feat length mismatch: alpha_feat={alpha_feat.shape}, "
                            f"shft_alpha_feat={None if shft_alpha_feat is None else shft_alpha_feat.shape}, "
                            f"cc={cc.shape}"
                        )
...
def early_fusion(curhs, model, model_name):
    if model_name in ["dkvmn","skvmn"]:
        ...
        p = p.squeeze(-1)
    elif model_name in ["akt","akt_ours","akt_dugp","extrakt", "folibikt","robustkt", "dtransformer","simplekt","stablekt","cskt", "fluckt", "datakt", "sparsekt", "lefokt_akt", "ukt", "hcgkt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx"]:
...
def effective_fusion(df, model, model_name, fusion_type):
    ...
    hasearly = ["dkvmn","deep_irt", "skvmn", "kqn", "akt","akt_ours","akt_dugp","extrakt", "folibikt", "robustkt", "dtransformer", "simplekt","stablekt","cskt","fluckt", "ukt", "hcgkt", "datakt", "sparsekt","lefokt_akt",  "saint", "sakt", "hawkes", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lpkt"]
    for ui in df:
        # 一题一题处理
        curdf = ui[1]
...
def group_fusion(dmerge, model, model_name, fusion_type, fout):
    ...
    hasearly = ["dkvmn","deep_irt", "skvmn", "kqn", "dtransformer", "akt","akt_ours","akt_dugp","robustkt", "extrakt", "folibikt","simplekt","stablekt","cskt", "fluckt", "ukt",  "hcgkt", "datakt", "sparsekt","lefokt_akt",  "saint", "sakt", "hawkes", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lpkt"]
...
def evaluate_question(model, test_loader, model_name, fusion_type=["early_fusion", "late_fusion"], save_path=""):
    ...
    hasearly = ["dkvmn","deep_irt", "skvmn", "kqn", "dtransformer", "akt","akt_ours","akt_dugp","extrakt","folibikt", "robustkt", "simplekt","cskt","fluckt", "stablekt", "ukt", "hcgkt", "datakt", "sparsekt", "lefokt_akt", "saint", "sakt", "hawkes", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lpkt"]
    if save_path != "":
        fout = open(save_path, "w", encoding="utf8")
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            # ours: causal group information
            group_id = dcurori.get("group_id", None)
            shft_group_id = dcurori.get("shft_group_id", None)

            alpha_feat = dcurori.get("alpha_feat", None)
            shft_alpha_feat = dcurori.get("shft_alpha_feat", None)
            ...
            if model_name in ["dkvmn","skvmn"]:
                ...
                y, h = model(dcurori, qtest=True, train=False)
            elif model_name in ["akt","akt_ours","akt_dugp","extrakt", "folibikt","fluckt","robustkt", "lefokt_akt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx"]:
...
def evaluate_splitpred_question(model, data_config, testf, model_name, save_path="", use_pred=False, train_ratio=0.2, atkt_pad=False):
    ...
    # ours: load uid -> group_id mapping for strict split prediction
    group_path = os.path.join(data_config["dpath"], "student_group_k8.csv")
    ...
    if os.path.exists(group_path):
        ...
        default_group_id = int(group_df["group_id"].mode().iloc[0])
        print(f"Loaded student group file for split prediction: {group_path}")
    else:
        uid2group = {}
        default_group_id = 0
        print(f"[Warning] {group_path} not found. Use group_id=0 for split prediction.")
    #============================================================
    ...
    with torch.no_grad():
        ...
        for i, row in df.iterrows():
            ...
            # ours: strict uid -> group_id
            uid_str = str(row["uid"]) if "uid" in row else None
            cur_group_id = uid2group.get(uid_str, default_group_id)
            #=============================================================
            ...
            if not use_pred:
                ...
                qidxs, ctrues, cpreds = predict_each_group2(dtotal, dcur, dforget, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad, group_id=cur_group_id)
                # 计算
                ...
                while t < curl:
                    ...
                    uid = row["uid"]
                    if model_name == "lpkt":
                        curqin, curcin, currin, curtin, curitin, curdforget, ctrues, cpreds = predict_each_group(dtotal, dcur, dforget, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad, group_id=cur_group_id)
                        dcur = {"curqin": curqin, "curcin": curcin, "currin": currin, "curtin": curtin, "curitin": curitin}
                    elif model_name == "dimkt":
                        curqin, curcin, currin, curtin, cursdin, curqdin, ctrues, cpreds = predict_each_group(dtotal, dcur, dforget, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad, group_id=cur_group_id)
                        dcur = {"curqin": curqin, "curcin": curcin, "currin": currin, "curtin": curtin, "cursdin": cursdin, "curqdin":curqdin}
                    else:
                        curqin, curcin, currin, curtin, curdforget, ctrues, cpreds = predict_each_group(dtotal, dcur, dforget, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad, group_id=cur_group_id)
                        dcur = {"curqin": curqin, "curcin": curcin, "currin": currin, "curtin": curtin}
...
def predict_each_group(dtotal, dcur, dforget, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad=False, group_id=None, maxlen=200):
    ...
    for k in range(t, end):
        ...
        if model_name in ["atdkt"]: ## need change!
            ...
            if model_name in ["akt_ours", "akt_dugp"]:
                if group_id is None:
                    cur_group_id = torch.zeros(
                        cin.size(0),
                        dtype=torch.long,
                        device=cin.device
                    )
                else:
                    cur_group_id = torch.full(
                        (cin.size(0),),
                        int(group_id),
                        dtype=torch.long,
                        device=cin.device
...
def predict_each_group2(dtotal, dcur, curdforget, dforget, is_repeat, qidx, uid, idx, curqin, curcin, currin, model_name, model, t, cq, cc, cr, end, fout, atkt_pad=False, group_id=None, maxlen=200):
    ...
    while bidx < finalcs.shape[0]:
        ...
        if model_name in ["atdkt"]:
            ...
            y = y[:, 1:]
        elif model_name in ["akt","akt_ours","akt_dugp","extrakt","folibikt", "robustkt", "cakt","fluckt","lefokt_akt",  "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx"]:                                
