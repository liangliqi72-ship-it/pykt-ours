...
def model_forward(model, data, rel=None):
    ...
    # ours: student group information
    ...
    if model_name in ["akt_ours", "akt_dugp"]:
        if group_id is not None:
            ...
            if cgroup_id.size(1) != cc.size(1):
                raise ValueError(
                    f"group_id length mismatch: {cgroup_id.size()} vs cc {cc.size()}"
                )
        ...
        if alpha_feat is not None:
            ...
            if calpha_feat.size(1) != cc.size(1):
                raise ValueError(
                    f"alpha_feat length mismatch: {calpha_feat.size()} vs cc {cc.size()}"
                )
